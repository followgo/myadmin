...
// ProductCategAPI 产品的类别API
type ProductCategAPI struct{}

// Get 根据 `uuid` 获取指定的对象
func (api *ProductCategAPI) Get(c echo.Context) error {
	categ := &model.ProductCateg{UUID: c.Param("uuid")}
	has, err := categ.Get()
	if err != nil {
...
// Select 列出所有选择的对象
func (api *ProductCategAPI) Select(c echo.Context) error {
	filter := new(orm.Filter)
	if err := c.Bind(filter); err != nil {
		return &echo.HTTPError{Code: http.StatusBadRequest, Message: "参数错误", Internal: err}
	}

	categories, err := new(model.ProductCateg).Find(filter)
	if err != nil {
		return &echo.HTTPError{Code: http.StatusInternalServerError, Message: "读取数据出错", Internal: err}
	}

	// 数量
	total, err := new(model.ProductCateg).Count(filter)
	if err != nil {
		return &echo.HTTPError{Code: http.StatusInternalServerError, Message: "读取数据出错", Internal: err}
	}
...
// Create 创建一个新对象
func (api *ProductCategAPI) Create(c echo.Context) error {
	categ := &model.ProductCateg{}
...
// Update 完全更新一个对象
func (api *ProductCategAPI) Update(c echo.Context) error {
	categ := &model.ProductCateg{}
	if err := c.Bind(categ); err != nil {
		return &echo.HTTPError{Code: http.StatusBadRequest, Message: "参数错误", Internal: err}
	}
...
// Patch 修改一个对象的属性
func (api *ProductCategAPI) Patch(c echo.Context) error {
	categ := &model.ProductCateg{}
	if err := c.Bind(categ); err != nil {
		return &echo.HTTPError{Code: http.StatusBadRequest, Message: "参数错误", Internal: err}
	}
...
// Delete 删除一个对象
func (api *ProductCategAPI) Delete(c echo.Context) error {
	categ := &model.ProductCateg{UUID: c.Param("uuid")}
	ok, err := categ.Del()
	if err != nil {
